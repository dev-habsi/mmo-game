...
type HudCallbacks = {
  ...
  onTrade: (input: TradeRequestInput) => void;
};
...
export function createHud(callbacks: HudCallbacks): HudController {
  ...
  const targetEl = must<HTMLInputElement>("#trade-target");
  const sendTradeEl = must<HTMLButtonElement>("#trade-send");
...
function createItemInputs(root: HTMLElement, prefix: string): void {
  root.innerHTML = "";
...
function readItems(prefix: string): Partial<Record<ResourceType, number>> {
  const items: Partial<Record<ResourceType, number>> = {};
  for (const type of RESOURCE_TYPES) {
    const value = Number(must<HTMLInputElement>(`#${prefix}-${type}`).value || 0);
    items[type] = Number.isFinite(value) ? Math.max(0, Math.floor(value)) : 0;
  }
  return items;
}

function labelForStructure(type: StructureType): string {
